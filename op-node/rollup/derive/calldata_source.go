...
type DataSourceFactory struct {
	log     log.Logger
	cfg     *rollup.Config
	daCfg   *txmgr.DAConfig
	fetcher L1TransactionFetcher
}

func NewDataSourceFactory(log log.Logger, cfg *rollup.Config, daCfg *txmgr.DAConfig, fetcher L1TransactionFetcher) *DataSourceFactory {
...
func NewDataSource(ctx context.Context, log log.Logger, cfg *rollup.Config, daCfg *txmgr.DAConfig, fetcher L1TransactionFetcher, block eth.BlockID, batcherAddr common.Address) (DataIter, error) {
...
func DataFromEVMTransactions(config *rollup.Config, daCfg *txmgr.DAConfig, batcherAddr common.Address, txs types.Transactions, log log.Logger) ([]eth.Data, error) {
	var out []eth.Data
	...
	for j, tx := range txs {
		if to := tx.To(); to != nil && *to == config.BatchInboxAddress {
			...
			if daCfg != nil {
				data, err := daCfg.GetDA(tx.Data())
				if err != nil {
					log.Warn("GetDA error", "key", string(tx.Data()), "err", err)
					return nil, err
				}
				bytes, err := hex.DecodeString(data)
				if err != nil {
					log.Warn("hex decode error: ", err)
					return nil, err
				}
